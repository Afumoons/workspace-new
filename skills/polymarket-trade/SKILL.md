...
This is read-only + paper trading. Real trading requires wallet authentication (not implemented in `polymarket.py`).

A separate helper script, `scripts/polymarket_wallet_trader.py`, adds **optional live trading on Polygon** using the official `py-clob-client` SDK. It:
- Uses **DRY_RUN = true** by default (no on-chain orders)
- Requires `POLYMARKET_PRIVATE_KEY` and `POLYMARKET_FUNDER` env vars for live mode
- Enforces `MAX_BET_USD`, `MAX_SESSION_USD`, and `RESERVE_USD` safety limits via `polymarket_wallet_config.py`

Example live-trading usage from `{baseDir}`:

```bash
# Check config and safety limits
python3 {baseDir}/scripts/polymarket_wallet_trader.py status

# Simulate a $25 buy on a market (DRY_RUN default)
python3 {baseDir}/scripts/polymarket_wallet_trader.py buy trump-2028 25

# After exporting POLYMARKET_PRIVATE_KEY / POLYMARKET_FUNDER and
# setting POLYMARKET_DRY_RUN=false, place a real order (YES tokens):
python3 {baseDir}/scripts/polymarket_wallet_trader.py buy trump-2028 25 --confirm-live
```
